...
import { pickCandidate, pickCurrentTurn, pickRecent } from "./session-surface.js";
import {
  BeatBudget,
  buildBeatSteering,
  decideBeat,
...
export async function installNarrativeLayer(ctx) {
  ...
  marks.ctx = ctx;
  // 注意：**不要**直接读 `ctx.llm` —— cordis 会用
  // `cannot get property "llm" without inject` 抛错（实测踩过，整个叙述层
  // 因此挂载失败）。必须先在 `inject` 里声明依赖，见下面那段。
  marks.llm = null;
  ...
  ctx.inject(["llm"], (scoped) => {
    ...
    marks.llmReady = true;
    console.log("[dsh-herta] llm 服务已就绪（做梦蒸馏可用）");
  });
    ...
    () =>
      ctx.on("tools/result", (exec, result) => {
        try {
          ...
          const turn = pickCurrentTurn({
            nodes: exec?.agent?.session?.surface?.nodes ?? [],
            eventAt: (seq) => exec?.agent?.session?.eventAt(seq),
          });
          if (turn === null) {
